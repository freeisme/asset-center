...
function renderPage() {
  ...
  if (state.page === "rackLayout") return renderRackLayoutPage();
...
document.addEventListener("click", (event) => {
  ...
  if (action === "navigate") {
    ...
    if (state.page === "rackLayout") {
      loadRackLayoutData({ force: true })
        .then(() => renderIfCurrentPage(targetPage))
        .catch((error) => {
          console.error("Unable to load rack layout", error);
          showToast(`机柜视图加载失败：${error.message}`, true);
        });
    }
...
const API_RACK_LAYOUT_RACKS = "/api/rack-layout/racks";
const API_RACK_LAYOUT_AVAILABLE = "/api/rack-layout/available";

const rackCategoryLabels = {
  server: "服务器",
  network: "网络",
  "patch-panel": "配线",
  power: "供电",
...
const rackFaceLabels = { front: "前面板", rear: "后面板", both: "整机深度" };
const rackStatusLabels = { in_use: "在用", idle: "闲置", repair: "维修", retired: "报废", lost: "丢失", shared: "公用" };
...
let rackLayoutState = {
  ...
  rack: null,
  placements: [],
  palette: { computers: [], inventoryModels: [] },
  face: "front",
  labels: true,
  uPx: 22,
  selectedId: "",
...
function rackLayoutRack() {
  return rackLayoutState.rack;
}

function rackOccupancy(ignoreId) {
  const map = new Map();
  (rackLayoutState.placements || []).forEach((placement) => {
    if (ignoreId && String(placement.id) === String(ignoreId)) return;
    for (let unit = placement.positionU; unit < placement.positionU + placement.uHeight; unit += 1) {
      map.set(unit, placement);
    }
  });
  return map;
}

function rackFaceVisible(placement) {
  if (rackLayoutState.face === "both") return true;
  if (placement.face === "both") return true;
  return placement.face === rackLayoutState.face;
}

function rackConflicts(positionU, uHeight, face, ignoreId) {
  const map = rackOccupancy(ignoreId);
  const hits = [];
  for (let unit = positionU; unit < positionU + uHeight; unit += 1) {
    const other = map.get(unit);
    if (!other) continue;
    const clash = face === "both" || other.face === "both" || other.face === face;
    if (clash && !hits.some((item) => String(item.id) === String(other.id))) hits.push(other);
  }
  return hits;
}

async function loadRackLayoutData({ force = false } = {}) {
  if (rackLayoutState.loading) return;
  if (rackLayoutState.loaded && !force) return;
  rackLayoutState.loading = true;
  try {
    const [racks, available] = await Promise.all([
      requestJson(API_RACK_LAYOUT_RACKS),
      requestJson(API_RACK_LAYOUT_AVAILABLE).catch(() => ({ computers: [], inventoryModels: [] })),
    ]);
    rackLayoutState.racks = Array.isArray(racks.racks) ? racks.racks : [];
    rackLayoutState.palette = {
      computers: Array.isArray(available.computers) ? available.computers : [],
      inventoryModels: Array.isArray(available.inventoryModels) ? available.inventoryModels : [],
    };
    rackLayoutState.loaded = true;
    const currentId = rackLayoutState.rack ? rackLayoutState.rack.id : "";
    const next = rackLayoutState.racks.find((rack) => String(rack.id) === String(currentId)) || rackLayoutState.racks[0];
    if (next) await loadRackLayout(next.id);
  } finally {
    rackLayoutState.loading = false;
  }
}

async function loadRackLayout(rackId) {
  if (!rackId) {
    rackLayoutState.rack = null;
    rackLayoutState.placements = [];
    return;
  }
  const payload = await requestJson(`${API_RACK_LAYOUT_RACKS}/${encodeURIComponent(rackId)}`);
  rackLayoutState.rack = payload.rack || null;
  rackLayoutState.placements = rackLayoutState.rack && Array.isArray(rackLayoutState.rack.placements)
    ? rackLayoutState.rack.placements
    : [];
  rackLayoutState.selectedId = "";
  rackLayoutState.armed = null;
}

async function refreshRackLayoutPalette(keyword = "") {
  const query = keyword ? `?keyword=${encodeURIComponent(keyword)}` : "";
  const payload = await requestJson(`${API_RACK_LAYOUT_AVAILABLE}${query}`);
  rackLayoutState.palette = {
    computers: Array.isArray(payload.computers) ? payload.computers : [],
    inventoryModels: Array.isArray(payload.inventoryModels) ? payload.inventoryModels : [],
  };
}

async function placeRackDevice(positionU) {
  const armed = rackLayoutState.armed;
  const rack = rackLayoutRack();
  if (!armed || !rack) return;
  const payload = {
    sourceKind: armed.sourceKind,
    positionU,
    uHeight: armed.uHeight || 1,
    face: "front",
    category: armed.category || "other",
  };
  if (armed.sourceKind === "computer") {
    payload.computerId = armed.computerId;
    payload.displayName = armed.name;
    payload.brandModel = armed.brandModel || "";
  } else {
    payload.displayName = armed.name;
    payload.brandModel = armed.brandModel || "";
    if (armed.inventoryModelId) payload.inventoryModelId = armed.inventoryModelId;
  }
  try {
    await runCommand(
      `${API_RACK_LAYOUT_RACKS}/${encodeURIComponent(rack.id)}/placements`,
      payload,
      "rack-placement-create",
    );
    rackLayoutState.armed = null;
    showToast(`已上架到 U${positionU}。`);
    await loadRackLayout(rack.id);
    await refreshRackLayoutPalette();
    render();
  } catch (error) {
    showToast(`上架失败：${error.message}`, true);
  }
}

async function moveRackDevice(placementId, payload, successMessage) {
  const rack = rackLayoutRack();
  if (!rack) return false;
  try {
    await requestJson(`/api/rack-layout/placements/${encodeURIComponent(placementId)}`, {
      method: "PUT",
      body: JSON.stringify(payload),
    });
    ...
    showToast(`保存机柜位置失败：${error.message}`, true);
    await loadRackLayout(rack.id);
    render();
    return false;
  }
...
async function removeRackDevice(placementId) {
  const rack = rackLayoutRack();
  const placement = (rackLayoutState.placements || []).find((item) => String(item.id) === String(placementId));
  if (!rack || !placement) return;
  const reason = window.prompt(`下架「${placement.name}」的原因（会写入操作日志）：`, "");
  if (reason === null) return;
  try {
    await runCommand(
      `/api/rack-layout/placements/${encodeURIComponent(placementId)}/remove`,
      { reason: String(reason).trim().slice(0, 200) },
      "rack-placement-remove",
    );
    showToast("设备已下架。");
    await loadRackLayout(rack.id);
    await refreshRackLayoutPalette();
    ...
    showToast(`下架失败：${error.message}`, true);
  }
}

function renderRackLayoutPage() {
  if (!hasPermission("rack_layout", "view")) {
    return '<div class="empty-state">当前账号没有机柜视图的查看权限。</div>';
  }
  const racks = rackLayoutState.racks || [];
  if (!racks.length) {
    return `<section class="page-intro"><div><span class="eyebrow">RACK LAYOUT</span><h2>机柜视图</h2>
        <p>把台账设备放到机柜的具体 U 位，可选前面板、后面板或整机深度。</p></div></section>
      <div class="empty-state">还没有机柜。请先在“机房巡检 → 机房与机柜”里维护机房和机柜。</div>`;
  }
  const rack = rackLayoutRack();
  const canCreate = hasPermission("rack_layout", "create");
  const canUpdate = hasPermission("rack_layout", "update");
  const canDelete = hasPermission("rack_layout", "delete");
  const placements = rackLayoutState.placements || [];
  const selected = placements.find((item) => String(item.id) === String(rackLayoutState.selectedId));
  const rackOptions = racks
    .map(
      (item) =>
        `<option value="${escapeHtml(item.id)}"${
          rack && String(item.id) === String(rack.id) ? " selected" : ""
        }>${escapeHtml(`${item.siteName} / ${item.name}（${item.heightU}U，已用 ${item.usedUnits}U）`)}</option>`,
    )
    .join("");
  return `<section class="page-intro">
      <div><span class="eyebrow">RACK LAYOUT</span><h2>机柜视图</h2>
        <p>拖动设备改 U 位，或点选右侧设备后点击空位上架；固资编码、使用人、状态以资产台账为准。</p></div>
    </section>
    <div class="toolbar">
      <select class="filter-select" data-action="rack-layout-select">${rackOptions}</select>
      <button class="secondary-button${rackLayoutState.face === "front" ? " is-active" : ""}" data-action="rack-layout-face" data-face="front">前面板</button>
      <button class="secondary-button${rackLayoutState.face === "rear" ? " is-active" : ""}" data-action="rack-layout-face" data-face="rear">后面板</button>
      <button class="secondary-button" data-action="rack-layout-labels">${rackLayoutState.labels ? "隐藏标签" : "显示标签"}</button>
      <span class="toolbar-spacer"></span>
      <button class="secondary-button" data-action="rack-layout-print">打印机柜图</button>
      <button class="secondary-button" data-action="rack-layout-export" ${placements.length ? "" : "disabled"}>导出设备清单</button>
      ${rack && hasPermission("inspection_management", "create") ? '<button class="secondary-button" data-action="rack-layout-inspect">发起巡检</button>' : ""}
    </div>
    ${rack ? renderRackLayoutBody(rack, placements, selected, { canCreate, canUpdate, canDelete }) : '<div class="empty-state">请选择机柜。</div>'}`;
}

function renderRackLayoutBody(rack, placements, selected, { canCreate, canUpdate, canDelete }) {
  const uPx = rackLayoutState.uPx;
  const height = Number(rack.heightU || 42);
  const occupied = rackOccupancy("");
  const unitLabels = [];
  for (let unit = height; unit >= 1; unit -= 1) {
    unitLabels.push(`<div class="rack-unit-label" style="height:${uPx}px">${unit}</div>`);
  }
  const blocks = placements
    .map((placement) => {
      const top = (height - (placement.positionU + placement.uHeight - 1)) * uPx + 1;
      const blockHeight = placement.uHeight * uPx - 2;
      const dimmed = rackFaceVisible(placement) ? "" : " is-dimmed";
      const label = rackLayoutState.labels ? placement.name : rackCategoryLabels[placement.category] || "设备";
      const detail =
        placement.uHeight >= 2 && placement.brandModel
          ? `<span class="rack-device-model">${escapeHtml(placement.brandModel)}</span>`
          : "";
      return `<div class="rack-device rack-status-${escapeHtml(placement.status || "other")}${
        String(placement.id) === String(rackLayoutState.selectedId) ? " is-selected" : ""
      }${dimmed}" data-device="${escapeHtml(placement.id)}" data-face="${escapeHtml(placement.face)}"
        style="top:${top}px;height:${blockHeight}px"${
          canUpdate ? "" : ' data-locked="1"'
        } title="${escapeHtml(`${placement.name} · ${placement.brandModel || ""}`)}">
        <span class="rack-device-name">${escapeHtml(label)}</span>
        ${detail}
        <span class="rack-device-tag">${escapeHtml(String(placement.uHeight))}U</span>
      </div>`;
    })
    .join("");
  const slots = [];
  if (canCreate || canUpdate) {
    for (let unit = height; unit >= 1; unit -= 1) {
      ...
      const top = (height - unit) * uPx + 1;
      slots.push(`<div class="rack-empty-slot" data-slot="${unit}" style="top:${top}px;height:${uPx - 1}px"></div>`);
    }
  }
  const used = rack.usedUnits ?? placements.reduce((total, item) => total + item.uHeight, 0);
  return `<div class="rack-layout-grid">
      <section class="data-panel">
        <div class="section-heading">
          <div><h2>${escapeHtml(rack.name)}</h2>
            <span>${escapeHtml(`${rack.siteName} ｜ ${height}U ｜ ${rackFaceLabels[rackLayoutState.face]}`)} ｜
            已用 ${escapeHtml(String(used))}U / 空闲 ${escapeHtml(String(Math.max(0, height - used)))}U</span></div>
        </div>
        <div class="rack-frame">
          <div class="rack-units">${unitLabels.join("")}</div>
          <div class="rack-body" data-rack="${escapeHtml(rack.id)}"
            style="--rack-u:${uPx}px;height:${height * uPx + 2}px">${slots.join("")}${blocks}</div>
          <div class="rack-units is-right">${unitLabels.join("")}</div>
        </div>
        <div class="rack-legend">
          <span><i class="rack-swatch rack-status-in_use"></i>在用</span>
          <span><i class="rack-swatch rack-status-idle"></i>闲置</span>
          <span><i class="rack-swatch rack-status-repair"></i>维修</span>
          <span><i class="rack-swatch rack-status-retired"></i>报废</span>
          <span>虚线边框＝后面板设备</span>
        </div>
      </section>
      <aside>
        ${renderRackInspector(selected, rack, { canUpdate, canDelete })}
        ${renderRackPalette({ canCreate })}
      </aside>
    </div>`;
}

function renderRackInspector(placement, rack, { canUpdate, canDelete }) {
  if (!placement) {
    return `<section class="data-panel"><div class="section-heading"><div><h2>设备属性</h2>
        <span>点击机柜里的设备查看或调整</span></div></div>
      <div class="rack-note">选中设备后可改起始 U 位、占用高度与安装面板；直接拖动设备也能换位置。</div></section>`;
  }
  const heights = [1, 2, 3, 4, 6, 8];
  return `<section class="data-panel">
    <div class="section-heading"><div><h2>设备属性</h2>
      <span>${escapeHtml(placement.sourceKind === "computer" ? "办公终端台账联动" : "手工登记")}</span></div>
      ${canDelete ? `<button class="text-button" data-action="rack-layout-remove" data-id="${escapeHtml(
        placement.id,
      )}">下架</button>` : ""}</div>
    <div class="rack-form">
      <label class="form-field"><span>显示名称</span>
        <input type="text" data-rack-field="displayName" value="${escapeHtml(placement.name)}"${
          canUpdate ? "" : " disabled"
        } /></label>
      <label class="form-field"><span>品牌型号</span>
        <input type="text" data-rack-field="brandModel" value="${escapeHtml(placement.brandModel || "")}"${
          canUpdate ? "" : " disabled"
        } /></label>
      <div class="rack-form-pair">
        <label class="form-field"><span>起始 U 位</span>
          <input type="number" min="1" max="${escapeHtml(String(rack.heightU || 42))}"
            data-rack-field="positionU" value="${escapeHtml(String(placement.positionU))}"${
              canUpdate ? "" : " disabled"
            } /></label>
        <label class="form-field"><span>占用高度</span>
          <select data-rack-field="uHeight"${canUpdate ? "" : " disabled"}>${heights
            .map(
              (value) =>
                `<option value="${value}"${value === placement.uHeight ? " selected" : ""}>${value}U</option>`,
            )
            .join("")}</select></label>
      </div>
      <label class="form-field"><span>设备类型</span>
        <select data-rack-field="category"${canUpdate ? "" : " disabled"}>${Object.entries(rackCategoryLabels)
          .map(
            ([value, label]) =>
              `<option value="${value}"${value === placement.category ? " selected" : ""}>${escapeHtml(label)}</option>`,
          )
          .join("")}</select></label>
      <label class="form-field"><span>安装面板</span>
        <select data-rack-field="face"${canUpdate ? "" : " disabled"}>${Object.entries(rackFaceLabels)
          .map(
            ([value, label]) =>
              `<option value="${value}"${value === placement.face ? " selected" : ""}>${escapeHtml(label)}</option>`,
          )
          .join("")}</select></label>
      <label class="form-field"><span>备注</span>
        <input type="text" data-rack-field="notes" value="${escapeHtml(placement.notes || "")}"${
          canUpdate ? "" : " disabled"
        } /></label>
    </div>
    <div class="rack-meta">
      <div><span>固资编码</span><strong>${escapeHtml(placement.assetCode || "—")}</strong></div>
      <div><span>SN / ST</span><strong>${escapeHtml(placement.serialNumber || "—")}</strong></div>
      <div><span>使用人</span><strong>${escapeHtml(placement.ownerLabel || "未分配")}</strong></div>
      <div><span>台账状态</span><strong>${escapeHtml(
        rackStatusLabels[placement.status] || placement.status || "—",
      )}</strong></div>
    </div>
    <div class="rack-note">这里改的是机柜位置；固资编码、SN、使用人和状态仍以资产台账为准。</div>
  </section>`;
}

function renderRackPalette({ canCreate }) {
  if (!canCreate) return "";
  const { computers, inventoryModels } = rackLayoutState.palette;
  const armed = rackLayoutState.armed;
  const rows = [
    ...(computers || []).map((item) => ({
      ...
      uHeight: 1,
      note: `办公终端 ｜ ${item.status ? rackStatusLabels[item.status] || item.status : ""}${
  ...
  return `<section class="data-panel" style="margin-top:12px">
    <div class="section-heading"><div><h2>未上架设备</h2>
      <span>办公终端来自台账，IT 物资按型号登记</span></div></div>
    <div class="rack-palette-search">
      <input type="search" data-rack-field="paletteSearch" placeholder="搜索设备名、编码或型号" />
    </div>
    <div class="rack-palette">${
      rows.length
        ? rows
            .map(
              (row) => `<button type="button" class="rack-palette-item${
                armed && armed.key === row.key ? " is-armed" : ""
              }" data-palette="${escapeHtml(row.key)}">
                <strong>${escapeHtml(row.name)}</strong>
                <small>${escapeHtml(row.brandModel || "")} ｜ ${escapeHtml(
                  rackCategoryLabels[row.category] || "其他",
                )} ｜ ${escapeHtml(row.note)}</small>
              </button>`,
            )
            .join("")
        : '<div class="rack-note">没有未上架的设备。</div>'
    }</div>
    <div class="rack-note">点选设备后，点击机柜里的空位即可上架；默认 1U，可在属性面板调整高度。</div>
  </section>`;
}

function rackLayoutSelected() {
  return (rackLayoutState.placements || []).find(
    (item) => String(item.id) === String(rackLayoutState.selectedId),
  );
}
...
document.addEventListener("click", (event) => {
  ...
  if (action.startsWith("rack-layout-")) {
    if (action === "rack-layout-face") {
      rackLayoutState.face = actionElement.dataset.face || "front";
      render();
      return;
    }
    if (action === "rack-layout-labels") {
      rackLayoutState.labels = !rackLayoutState.labels;
      render();
      return;
    }
    if (action === "rack-layout-print") {
      printRackLayout();
      return;
    }
    if (action === "rack-layout-export") {
      exportRackLayout();
      return;
    }
    if (action === "rack-layout-inspect") {
      const rack = rackLayoutRack();
      if (!rack) return;
      ...
      loadInspectionData({ force: true })
        .then(() => {
          render();
          openInspectionStartModal({ rackCode: rack.code });
        })
        .catch((error) => showToast(`巡检模块加载失败：${error.message}`, true));
      return;
    }
    if (action === "rack-layout-remove") {
      removeRackDevice(actionElement.dataset.id || "");
      return;
    }
    if (action === "rack-layout-refresh") {
      loadRackLayoutData({ force: true })
        .then(() => render())
        .catch((error) => showToast(`机柜数据加载失败：${error.message}`, true));
      return;
    }
  }

  const paletteItem = event.target.closest("[data-palette]");
  if (paletteItem && state.page === "rackLayout") {
    const key = paletteItem.dataset.palette || "";
    const { computers, inventoryModels } = rackLayoutState.palette;
    const armed = key.startsWith("computer:")
      ? (computers || []).find((item) => `computer:${item.computerId}` === key)
      : (inventoryModels || []).find((item) => `model:${item.inventoryModelId}` === key);
    if (armed) {
      rackLayoutState.armed = {
        key,
        sourceKind: key.startsWith("computer:") ? "computer" : "custom",
        computerId: armed.computerId,
        inventoryModelId: armed.inventoryModelId,
        name: armed.name,
        brandModel: armed.brandModel,
        category: armed.category,
        uHeight: armed.uHeight || 1,
      };
      showToast(`已选中「${armed.name}」：点击机柜里的空位放置。`);
      render();
    }
    return;
  }
  ...
  if (slot && state.page === "rackLayout") {
    if (!rackLayoutState.armed) {
      showToast("先在右侧“未上架设备”里选一台设备。", true);
      return;
    }
    placeRackDevice(Number(slot.dataset.slot));
    return;
  }

  const block = event.target.closest("[data-device]");
  if (block && state.page === "rackLayout") {
    rackLayoutState.selectedId = block.dataset.device;
    const placement = rackLayoutSelected();
    if (placement) showToast(`已选中「${placement.name}」：可拖动换 U 位，或在右侧改属性。`);
    render();
  }
});

document.addEventListener("change", (event) => {
  const select = event.target.closest('[data-action="rack-layout-select"]');
  if (select && state.page === "rackLayout") {
    loadRackLayout(select.value)
      .then(() => render())
      .catch((error) => showToast(`机柜加载失败：${error.message}`, true));
    return;
  }
  const field = event.target.dataset.rackField;
  if (!field || state.page !== "rackLayout" || field === "paletteSearch") return;
  const placement = rackLayoutSelected();
  const rack = rackLayoutRack();
  if (!placement || !rack) return;
  const payload = {
    rackId: rack.id,
    displayName: field === "displayName" ? event.target.value.trim() || placement.name : placement.name,
    brandModel: field === "brandModel" ? event.target.value.trim() : placement.brandModel,
    category: field === "category" ? event.target.value : placement.category,
    positionU: field === "positionU" ? Number(event.target.value) : placement.positionU,
    uHeight: field === "uHeight" ? Number(event.target.value) : placement.uHeight,
    face: field === "face" ? event.target.value : placement.face,
    notes: field === "notes" ? event.target.value.trim() : placement.notes,
  };
  const labels = {
    positionU: `已移动到 U${payload.positionU}。`,
    uHeight: `占用高度已改为 ${payload.uHeight}U。`,
    face: `安装面板已改为${rackFaceLabels[payload.face] || payload.face}。`,
  };
  moveRackDevice(placement.id, payload, labels[field]);
});

document.addEventListener("input", (event) => {
  if (state.page !== "rackLayout") return;
  if (event.target.dataset.rackField !== "paletteSearch") return;
  const keyword = event.target.value.trim();
  window.clearTimeout(rackLayoutState.searchTimer);
  rackLayoutState.searchTimer = window.setTimeout(() => {
    refreshRackLayoutPalette(keyword)
      .then(() => render())
      .catch(() => {});
  }, 300);
});
...
document.addEventListener("pointerdown", (event) => {
  if (state.page !== "rackLayout") return;
  const block = event.target.closest("[data-device]");
  if (!block || block.dataset.locked) return;
  const placement = (rackLayoutState.placements || []).find(
    (item) => String(item.id) === String(block.dataset.device),
  );
  if (!placement) return;
  rackLayoutState.selectedId = placement.id;
  rackDrag = { placement, block, startY: event.clientY, startPosition: placement.positionU, moved: false };
  try {
...
document.addEventListener("pointermove", (event) => {
  ...
  const uPx = rackLayoutState.uPx;
  const height = Number(rack.heightU || 42);
  const deltaRows = Math.round((event.clientY - rackDrag.startY) / uPx);
  const candidate = Math.min(
    height - rackDrag.placement.uHeight + 1,
    Math.max(1, rackDrag.startPosition - deltaRows),
  );
  if (candidate !== rackDrag.placement.positionU) rackDrag.moved = true;
  rackDrag.candidate = candidate;
  const top = (height - (candidate + rackDrag.placement.uHeight - 1)) * uPx + 1;
  rackDrag.block.style.top = `${top}px`;
  const hits = rackConflicts(
    candidate,
    rackDrag.placement.uHeight,
    rackDrag.placement.face,
    rackDrag.placement.id,
  );
  rackDrag.block.classList.toggle("is-conflict", hits.length > 0);
  rackDrag.block.classList.add("is-dragging");
});

document.addEventListener("pointerup", () => {
  if (!rackDrag) return;
  const { placement, block, candidate, moved } = rackDrag;
  const rack = rackLayoutRack();
  block.classList.remove("is-dragging", "is-conflict");
  rackDrag = null;
  if (!rack || !moved || candidate === undefined) {
    render();
    return;
  }
  if (candidate === placement.positionU) {
    render();
    return;
  }
  moveRackDevice(
    ...
    `「${placement.name}」已移动到 U${candidate}。`,
  );
});
...
function exportRackLayout() {
  const rack = rackLayoutRack();
  const placements = rackLayoutState.placements || [];
  if (!rack || !placements.length) return showToast("当前机柜还没有上架设备。", true);
  downloadExcel(`机柜视图-${rack.name}-${exportDateStamp()}.xls`, [
    {
      name: "机柜设备",
      headers: ["起始U", "占用", "安装面", "设备名称", "品牌型号", "类型", "状态", "固资编码", "SN/ST", "使用人", "备注"],
      rows: placements
        .slice()
        ...
        .map((placement) => [
          ...
          `${placement.uHeight}U`,
          rackFaceLabels[placement.face] || placement.face,
          placement.name,
          placement.brandModel || "",
          rackCategoryLabels[placement.category] || placement.category,
          rackStatusLabels[placement.status] || placement.status || "",
          placement.assetCode || "",
...
function printRackLayout() {
  const rack = rackLayoutRack();
  const placements = rackLayoutState.placements || [];
  if (!rack) return showToast("请先选择机柜。", true);
  const height = Number(rack.heightU || 42);
  const occupied = rackOccupancy("");
  const rows = [];
  for (let unit = height; unit >= 1; unit -= 1) {
    const placement = occupied.get(unit);
    const isStart = placement && placement.positionU === unit;
    rows.push(
      `<tr><td class="u">${unit}U</td><td>${
        placement
          ? isStart
            ? `${escapeHtml(placement.name)} <small>${escapeHtml(placement.brandModel || "")} · ${
                rackFaceLabels[placement.face] || placement.face
              } · ${placement.uHeight}U</small>`
            : ""
          : ""
      }</td></tr>`,
    );
  ...
  if (!printWindow) return showToast("浏览器拦截了打印窗口，请允许弹出窗口后重试。", true);
  printWindow.document.write(
    `<html><head><meta charset="utf-8" /><title>${escapeHtml(`${rack.name} 机柜图`)}</title>
      <style>
        body { font-family: "Microsoft YaHei", Arial, sans-serif; font-size: 12px; margin: 24px; }
        h1 { font-size: 18px; margin: 0 0 4px; }
        p.meta { color: #555; margin: 0 0 12px; }
        table { border-collapse: collapse; width: 420px; }
        td { border: 1px solid #999; padding: 4px 6px; height: 18px; }
        td.u { width: 48px; text-align: right; color: #555; background: #f2f2f2; }
        small { color: #555; }
      </style></head><body>
      <h1>${escapeHtml(rack.name)}</h1>
      <p class="meta">${escapeHtml(rack.siteName)} ｜ ${height}U ｜ 设备 ${placements.length} 台 ｜ 打印时间 ${escapeHtml(
